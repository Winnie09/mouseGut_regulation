rm(list=ls())
## first group: TPM
tb1 = read.csv('data/rna/counts.raw.original_samples.csv', row.names = 1)
summary(tb1[,3])
summary(colSums(tb1))

meta = read.csv('data/meta/postnatal_meta_rnaseq_wh.csv')
rownames(meta) = meta[,2] 
str(meta)

## second group: counts
tb2 = read.csv('data/rna/counts_and_Rsq_for_new_data_wh.csv', row.names = 1)
str(tb2)


tb3 = read.csv('data/rna/HSCR_samples_wh.csv', row.names = 1)
tb3 = tb3[, 4:ncol(tb3)]
str(tb3)
...
pdf('qc/first_two_groups_samples/plot/hist_expr_temporal.pdf', width = 5, height = 3.5)
...
pdf('qc/first_two_groups_samples/plot/hist_expr_temporal_after_filtering.pdf', width = 5, height = 3.5)
hist(tmp[tmp!=0], breaks = 100, xlab = 'Gene expression (log2(TPM+1))', main = 'Histogram of non-zero gene expression values')
dev.off()
...
pr = PCA(expr)
str(pr) # 24, 9
saveRDS(pr, 'qc/first_two_groups_samples/plot/pr.rds')
...
pdf('qc/first_two_groups_samples/plot/RNAseq_pca_timepoint.pdf', width = 2.8, height = 1.8)
...
pdf('qc/first_two_groups_samples/plot/RNAseq_pca_samplegroups.pdf', width = 2.6, height = 1.8)
...
colnames(meta)
pdf('qc/first_two_groups_samples/plot/RNAseq_pca_libprep.pdf', width = 2.6, height = 1.8)
...
pdf('qc/first_two_groups_samples/plot/RNAseq_pca_pairedend.pdf', width = 2.6, height = 1.8)
...
saveRDS(fdr, 'qc/first_two_groups_samples/res/RNAseq_fdr.rds')
...
str(clu)
table(clu)
saveRDS(clu, 'qc/first_two_groups_samples/res/dynamic_genes_cluster_10clu.rds')
...
head(res.tb)
saveRDS(res.tb, 'qc/first_two_groups_samples/res/dynamic_genes_10clu.rds')


write.table(res.tb, 'qc/first_two_groups_samples/res/dynamic_genes_10clu.csv', row.names = F, sep = ',')
...
pdf('qc/first_two_groups_samples/plot/hm_diffgene.pdf', width = 4.5, height = 4.5)
...
saveRDS(resList, 'qc/first_two_groups_samples/plot/GOres.rds')


library(reshape2)
source('/Users/wenpinhou/Dropbox/trajectory_variability/package/Lamian/R/plotGOEnrich.R')
pdf('qc/first_two_groups_samples/plot/hm_diffgene_GO.pdf', width = 9, height = 10)
...
pdf('qc/first_two_groups_samples/plot/diffgene_cluster_pattern.pdf', width = 7, height = 4)
...
pdf('qc/first_two_groups_samples/plot/diffgene_dev.pdf', width = 7, height = 4)
...
head(pd)
colnames(pd) = c('time', 'gene', 'expression')
pdf('qc/first_two_groups_samples/plot/diffgene_top.pdf', width = 12, height = 6)
ggplot(data = pd, aes(x = time, y = expression)) +
  geom_point(size = 0.5) +
